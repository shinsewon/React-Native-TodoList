...
export default class AddListModal extends Component {
  ...
  renderColors() {
    return this.backgroundColors.map((item) => {
      return <TouchableOpacity key={item} style={[styles.colorSelect, { backgroundColor: item }]} onPress={() => this.setState({ color: item })} />;
    });
  }
    ...
    return (
      <KeyboardAvoidingView style={styles.container} behavior="padding">
        ...
        <View style={{ alignSelf: 'stretch', marginHorizontal: 32 }}>
          ...
          <TextInput style={styles.input} placeholder="할 일을 적어주세요." onChangeText={(e) => this.setState({ name: e })} />
          ...
          <TouchableOpacity style={[styles.create, { backgroundColor: this.state.color }]} onPress={this.createTodo}>
            <Text style={{ color: colors.white, fontWeight: '600' }}>생 성</Text>
          </TouchableOpacity>
        </View>
